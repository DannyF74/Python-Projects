...
# Author:       Danny Firth
#
# Purpose:      Student Tracker project. Learning OOP, Tkinter GUI module,
...
# Catch of the user's clicks on the windows upper-right 'X' to ensure they want to close
def ask_quit(self):
...
def create_db(self):
    ...
    with conn:
        cur = conn.cursor()
        conn.execute("CREATE TABLE if not exists tbl_tracker( \
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_tracker (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Software Developer'))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # Removes any blank spaces before or after the entry
    var_lname = var_lname.strip()
    var_fname = var_fname.title() # Capitalizes the first letter of the entry
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine our normalized names
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0): # enforce the user to provide both names
        ...
        messagebox.showerror("Missing Text Error","Please insure that there is data in all four fields.")
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT(col_phone) FROM tbl_tracker WHERE col_phone = '{}'""".format(var_phone))
            count = cur.fetchone()[0]
            print(count)
            cur.execute("""SELECT COUNT(col_email) FROM tbl_tracker WHERE col_email = '{}'""".format(var_email))
            count2 = cur.fetchone()[0]
            print(count2)
            cur.execute("""SELECT COUNT(col_course) FROM tbl_tracker WHERE col_course = '{}'""".format(var_course))
            count3 = cur.fetchone()[0]
            print(count3)
            if count == 0 or count2 == 0 or count3 == 0: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request","The following changes ({}), ({}) and ({}) will be implemented for ({}). \n\nPreceed with the update request?".format(var_phone,var_email,var_course,var_value))
                print(response)
                if response:
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_tracker SET col_phone = '{0}',col_email = '{1}',col_course = '{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_course,var_value))
                        onClear(self)
                        conn.commit()
                else:
        ...
        messagebox.showerror("Missing information","Please select a name from the list. \nThen edit the phone email or course information. Please do not leave any fields blank.")
    onClear(self)
